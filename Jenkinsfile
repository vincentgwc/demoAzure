pipeline {
    ...
    stages {
        ...
        stage('Build Docker image') {
            steps {            
                bat 'mvn -s C:/WorkspaceDemo/demo/artifactory-settings.xml clean verify com.google.cloud.tools:jib-maven-plugin:2.8.0:dockerBuild -DskipTests'
            }
        }
        
        stage('Push Docker image to Jfrog artifactory') {
            steps {     
                bat 'docker tag vgdockerimg vgjfrog.jfrog.io/vgdocker-docker/vgdemo:latest'
                ...
                bat 'docker push vgjfrog.jfrog.io/vgdocker-docker/vgdemo:latest'
            }
        }
        ...
        stage('Connect to Azure Kubernetes'){
        	steps {
        		...
        		bat 'kubectl set image deployments/vgdemo-app vgdemo-app=vgjfrog.jfrog.io/vgdocker-docker/vgdemo:latest'
